...
# class NewPostForm(forms.ModelForm):
    ...
    # login = forms.SlugField(max_length=255, label='URL')
    # content = forms.CharField(widget=forms.Textarea(attrs={'cols': 60, 'rows': 10}), label='Content')
    # photo = forms.ImageField(required=False)
    # is_published = forms.BooleanField(label='To publish?', required=False, initial=True)
    # creator = forms.ModelChoiceField(queryset=User.objects.all(), label='Category',
    #                                  empty_label='Choice creator ')
